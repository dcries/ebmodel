...
<<classical_lm_ex,echo=FALSE,fig.cap='Simulated data with observed X (solid dots) and W (diamonds) along with fitted OLS lines. Solid is for the true X, and dashed is for the W.',fig.width=4,fig.height=4 >>=
set.seed(5)
n <- 30
eps <- rnorm(n,0,0.5)
u <- rnorm(n,0,1)
...
ggplot() + geom_point(aes(x=x,y=y)) + geom_point(aes(x=w,y=y),shape=5) + geom_line(aes(x=x,y=fitted(lm1))) + geom_line(aes(x=w,y=fitted(lm2)),linetype=2) + theme_bw()
