...
chain1=mcmc_full(yee,yes,wee,wes,Z,initial,prior,nreps,burn,h,maxkt,my_bs,my_qp)
chain2=mcmc_full(yee,yes,wee,wes,Z,initial,prior,nreps,burn,h,maxkt,my_bs,my_qp)
chain3=mcmc_full(yee,yes,wee,wes,Z,initial,prior,nreps,burn,h,maxkt,my_bs,my_qp)
...
pi <- mcmc.list(mcmc(chain1$pi),mcmc(chain2$pi),mcmc(chain3$pi))
...
alpha <- mcmc.list(mcmc(chain1$alpha),mcmc(chain2$alpha),mcmc(chain3$alpha))
...
ppan=pp_full(chain1,truth,wsum,ysum,xee,xes,yeeb,yesb,Z)

out <- list(chain1=chain1,chain2=chain2,chain3=chain3,ppan=ppan)
save(out,file="//my.files.iastate.edu/Users/dcries/Desktop/dp_mcmc1.RData")
#save.image("//my.files.iastate.edu/Users/dcries/Desktop/dp_mcmc1.RData")
...
#unique components
table(apply(chain1$zeta,1,function(x){return(length(unique(x)))}))
#max utilized component, for DP
table(apply(chain1$zeta,1,function(x){return(max(x))}))

summat <- matrix(0,nrow=10,ncol=5)
#summat[1,] <- summary(muee)$quantiles
#summat[2,] <- summary(mues)$quantiles
summat[1,] <- quantile(sqrt(unlist(sigma2eee)),probs=c(0.025,0.25,0.5,0.75,0.975))
summat[2,] <- quantile(sqrt(unlist(sigma2ees)),probs=c(0.025,0.25,0.5,0.75,0.975))
summat[3,] <- quantile(sqrt(unlist(sigma2vee)),probs=c(0.025,0.25,0.5,0.75,0.975))
summat[4,] <- quantile(sqrt(unlist(sigma2ves)),probs=c(0.025,0.25,0.5,0.75,0.975))
...
#summat[5,] <- summary(corrx)$quantiles
summat[5:7,] <- summary(betaee)$quantiles
summat[8:10,] <- summary(betaes)$quantiles

summat <- data.frame(summat)
names(summat) <- c("2.5\\%","25\\%","50\\%","75\\%","97.5\\%")
row.names(summat) <- c("$\\sigma_{yee}$","$\\sigma_{yes}$",
                       "$\\sigma_{wee}$","$\\sigma_{wes}$",
...
#check DP density convergence, need 3 chains, rbind results together
mee <- rbind((chain1$muee),(chain2$muee),(chain3$muee))
see <- rbind(sqrt(chain1$sigma2xee),sqrt(chain2$sigma2xee),sqrt(chain3$sigma2xee))
pi2 <- rbind((chain1$pi),(chain2$pi),(chain3$pi))
mes <- rbind((chain1$mues),(chain2$mues),(chain3$mues))
ses <- rbind(sqrt(chain1$sigma2xes),sqrt(chain2$sigma2xes),sqrt(chain3$sigma2xes))

loc_check <- seq(1300,3300,length=10)
density_conv(cbind(mee,pi2,see),loc_check[3])

loc_check <- seq(-500,500,length=10)
density_conv(cbind(mes,pi2,ses),loc_check[2])
